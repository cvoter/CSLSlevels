...
#'
#' Given the cslsLevels data frame, calculate the mean number of times per year a given
#' lake can be expected to become more extreme than given exceedances
#'
...
#' @param startDate Specify a range of dates by which to filter lake level predictions
#' @param endDate Specify a range of dates by which to filter lake level predictions
#'
#' @return a list of the following:
#' \item{result} {A data frame of one column, with mean and cv for each exceedance supplied, with appropriate row names}
#' \item{df} {The data frame generated by hi_lo_duration() that was used by this function}
#'
#' @import magrittr
#' @import stats
#'
...
hi_lo_ann <- function(lakeName,
  ...
  for (e in paste0(exceeds*100, "%")) {
    ...
    countTables[[e]] <- table(table(bb$year))

    mean <- mean(table(bb$year))
    cv <- 100*sd(table(bb$year))/mean(table(bb$year))
