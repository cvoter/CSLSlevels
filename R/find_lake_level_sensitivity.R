...
find_lake_level_sensitivity <- function(levels, years, probs = c(10, 50, 90),
  ...
  ranked <- rank_lake_levels_sensitivity(levels, years, types, windows)
    ...
    if (year %in% unique(ranked$year)) {
      this_year <- ranked %>%
                   filter(.data$year == !!year,
                          ...
                          .data$window == !!window)
      for (prob in probs){
        df <- rbind(df, find_lake_level(this_year, year, type, window, prob))
      }
    }
  }
  return(df)
