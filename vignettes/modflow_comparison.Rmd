---
title: "MODFLOW comparison"
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{MODFLOW comparison}
...
# MODFLOW LLIRA baseline runs
MODFLOW         <- CSLSdata::MODFLOW
scenarios       <- c("irr", "no_irr")
MODFLOW_metrics <- NULL
for (scenario in scenarios) {
  for (sim in 1:max(MODFLOW$sim)) {
    message(sprintf("Starting sim %s", sim))
    this_MODFLOW <- MODFLOW %>% 
                    filter(.data$scenario == scenario,
                           .data$sim == sim) %>%
                    select(lake = .data$lake,
                           date = .data$date,
                           level = .data$level_m)
    these_metrics     <- calculate_metrics(this_MODFLOW, 
                                           metrics = c("exceedance_level",
                                                       "exceedance_range",
                                                       "num_2yr_seas",
                                                       "good_spawning"))
    these_metrics$scenario <- scenario
    these_metrics$sim      <- sim
    MODFLOW_metrics   <- rbind(MODFLOW_metrics, these_metrics)
  }
}
MODFLOW_metrics     <- MODFLOW_metrics %>%
                       mutate(value = ifelse(.data$metric %in% c("exceedance_level",
                                                                 "exceedance_range",
                                                                 "median_fall_rate",
                                                                 "median_rise_rate",
                                                                 "median_level"),
...
MODFLOW_metrics      <- dcast(MODFLOW_metrics, 
                              lake+metric+variable+sim~scenario, 
                              value.var = "value")
MODFLOW_metrics$diff <- MODFLOW_metrics$irr - MODFLOW_metrics$no_irr

# significance <- left_join(significance, MODFLOW_metrics, 
#                           by = c("lake", "metric", "variable"))
# significance$no_pumping <- significance$median - significance$diff

```

```{r eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=6.5, fig.height=4.5}
...
for (lake in lakes) {
  ...
  ggsave(sprintf("%s_significance_MODFLOW.png", lake), 
