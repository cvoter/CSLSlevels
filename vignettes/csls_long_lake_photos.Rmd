...
lake_levels <- csls_levels %>% filter(.data$lake == !!lake)
long_limits <- c(min(lake_levels$level_pred, na.rm = TRUE),
                 max(lake_levels$level_pred, na.rm = TRUE))

lake_levels_2000s  <- lake_levels %>%
                      filter(year(date) > 1999,
                             year(date) <= 2012)
lake_levels_1960s  <- lake_levels %>%
                      filter(year(date) > 1955,
                             year(date) <= 1968)
photo_levels_2000s <- lake_levels %>%
                      filter(date %in% photos_all_2000s,
                             !is.na(level_obs))
photo_levels_1960s <- lake_levels %>%
                      filter(date %in% photos_1964,
                             !is.na(level_obs))

# Exceedance probabilities
ranked_annual           <- calculate_probs_of_levels(lake_levels)
photo_levels_1960s$prob <- approx(x = ranked_annual$level,
                                  y = ranked_annual$prob,
                                  xout = photo_levels_1960s$level_obs)$y
photo_levels_2000s$prob <- approx(x = ranked_annual$level,
                                  y = ranked_annual$prob,
                                  xout = photo_levels_2000s$level_obs)$y

# Plotting functions
plot_historical_series <- function(df_obs, df_photos){
  plot_obj <- ggplot() +
              geom_line(data = df_obs,
                        aes(x = date, y = level_pred)) +
              geom_point(data = df_obs,
                         aes(x = date, y = level_obs)) +
...
> They were told the lake is a little low but it always comes back. That’s where
...
plot_bathy <- function(photo_dates, max_depth_lims, obs_loc = NULL) {
  lake_obs   <- lake_levels %>%
                filter(date %in% photo_dates)
  lake_level <- mean(lake_obs$level_obs, na.rm = TRUE)
