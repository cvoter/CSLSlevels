...
```{r pfl, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
lake <- "Plainfield"
...
lake_levels <- lake_levels %>%
               filter(.data$date == as_datetime(mdy("08-07-2018")))
...
exceedance  <- calculate_exceedances(lake_levels, probs = 50)
exceedance  <- exceedance %>%
               filter(.data$lake == !!lake)
lake_level_ex2  <- exceedance$value
# Specify
lake_level_ex3  <- 333

# Plot veg outlines at lake level.
# Can adjust other aesthetics as well (e.g. text size, legend position, etc.)
plot_veg_outlines(lake, lake_level_ex1, deepest, shallowest, contour_interval)
...
```{r long, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
lake <- "Long"
...
```{r psnt, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
lake <- "Pleasant"
